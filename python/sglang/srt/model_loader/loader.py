    ...
    def _move_and_quantize_per_module(
        ...
        for module in model.modules():
            ...
            if quant_method is not None:
                ...
                processed += 1
                if processed % 8 == 0:
                    # Synchronize before empty_cache to ensure all async device
                    # operations complete; without this, large MoE models (many
                    # experts = many modules) can hang on XPU as the async queue
                    # grows unboundedly.
                    current_platform.synchronize()
                    gc.collect()
                    current_platform.empty_cache()

        # Final sync + cache clear for any remaining operations
        current_platform.synchronize()
        gc.collect()
        current_platform.empty_cache()
